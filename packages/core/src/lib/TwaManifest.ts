...
export class TwaManifest {
    ...
    constructor(data: TwaManifestJson) {
        ...
        this.navigationColorDark = new Color(
            data.navigationColorDark ?? DEFAULT_NAVIGATION_COLOR
        );
        this.navigationDividerColor = new Color(
            data.navigationDividerColor ?? DEFAULT_NAVIGATION_DIVIDER_COLOR
        );
        this.navigationDividerColorDark = new Color(
            data.navigationDividerColorDark ?? DEFAULT_NAVIGATION_COLOR
        );
        ...
        this.shortcuts = (data.shortcuts || []).map((si) => {
            return new ShortcutInfo(
                ...
                si.url,
                si.chosenIconUrl,
                si.chosenMaskableIconUrl,
                si.chosenMonochromeIconUrl
            );
        });
        this.generatorApp = data.generatorApp || DEFAULT_GENERATOR_APP_NAME;
        this.webManifestUrl = data.webManifestUrl
            ? new URL(data.webManifestUrl)
            : undefined;
        this.fallbackType = data.fallbackType || "customtabs";
        this.features = data.features || {};
        this.alphaDependencies = data.alphaDependencies || { enabled: false };
        this.enableSiteSettingsShortcut =
            data.enableSiteSettingsShortcut != undefined
                ? data.enableSiteSettingsShortcut
                : true;
        this.isChromeOSOnly =
            data.isChromeOSOnly != undefined ? data.isChromeOSOnly : false;
    ...
    generateShortcuts(): string {
        return (
            "[" +
            this.shortcuts
                .map((shortcut, i) => shortcut.toString(i))
                .join(",") +
            "]"
        );
    }
     ...
     * @returns {TwaManifest}
     */
    static fromWebManifestJson(
        webManifestUrl: URL,
        webManifest: WebManifestJson
    ): TwaManifest {
        const icon = findSuitableIcon(webManifest.icons, "any", MIN_ICON_SIZE);
        const maskableIcon = findSuitableIcon(
            webManifest.icons,
            "maskable",
            MIN_ICON_SIZE
        );
        const monochromeIcon = findSuitableIcon(
            webManifest.icons,
            "monochrome",
            MIN_NOTIFICATION_ICON_SIZE
        );

        const fullStartUrl: URL = new URL(
            webManifest["start_url"] || "/",
            webManifestUrl
        );
        const shortcuts: ShortcutInfo[] = this.getShortcuts(
            webManifestUrl,
            webManifest
        );

        function resolveIconUrl(
            icon: WebManifestIcon | null
        ): string | undefined {
            return icon
                ? new URL(icon.src, webManifestUrl).toString()
                : undefined;
        }
        ...
        const twaManifest = new TwaManifest({
            ...
            launcherName:
                ...
                DEFAULT_APP_NAME,
            display:
                asDisplayMode(webManifest["display"]!) || DEFAULT_DISPLAY_MODE,
            ...
            backgroundColor:
                webManifest["background_color"] || DEFAULT_BACKGROUND_COLOR,
            ...
            shareTarget: TwaManifest.verifyShareTarget(
                webManifestUrl,
                webManifest.share_target
            ),
            orientation:
                asOrientation(webManifest.orientation) || DEFAULT_ORIENTATION,
        });
        return twaManifest;
    }
     ...
     * @returns {TwaManifest}
     */
    static async fromWebManifest(
        url: string,
        login?: string,
        password?: string
    ): Promise<TwaManifest> {
        console.warn("HERE", login, password);
        try {
            const response = await fetchUtils.fetch(url, login, password);
            console.log("response", response, login, password);
    ...
    static getShortcuts(
        ...
        for (let i = 0; i < (webManifest.shortcuts || []).length; i++) {
            ...
            try {
                const shortcutInfo = ShortcutInfo.fromShortcutJson(
                    webManifestUrl,
                    s
                );
    ...
    static async merge(
        ...
        const iconUrl = this.getNewIconUrl(
            fieldsToIgnore,
            "icons",
            oldTwaManifestJson.iconUrl!,
            webManifest.icons!,
            "any",
            MIN_ICON_SIZE,
            webManifestUrl
        );
        const maskableIconUrl = this.getNewIconUrl(
            fieldsToIgnore,
            "maskableIcons",
            oldTwaManifestJson.iconUrl!,
            webManifest.icons!,
        ...
        const monochromeIconUrl = this.getNewIconUrl(
            fieldsToIgnore,
            "monochromeIcons",
            oldTwaManifestJson.iconUrl!,
            webManifest.icons!,
        ...
        const twaManifest = new TwaManifest({
            ...oldTwaManifestJson,
            name: this.getNewFieldValue(
                "name",
                fieldsToIgnore,
                oldTwaManifest.name,
                webManifest["name"] || webManifest["short_name"]!
            ),
            launcherName: this.getNewFieldValue(
                "short_name",
                fieldsToIgnore,
                oldTwaManifest.launcherName,
                webManifest["short_name"] ||
                    webManifest["name"]?.substring(0, SHORT_NAME_MAX_SIZE)
            ),
            display: this.getNewFieldValue(
                "display",
                fieldsToIgnore,
                oldTwaManifest.display,
                asDisplayMode(webManifest["display"]!)!
            ),
            themeColor: this.getNewFieldValue(
                "themeColor",
                fieldsToIgnore,
                oldTwaManifest.themeColor.hex(),
                webManifest["theme_color"]!
            ),
            backgroundColor: this.getNewFieldValue(
                "backgroundColor",
                fieldsToIgnore,
                oldTwaManifest.backgroundColor.hex(),
                webManifest["background_color"]!
            ),
            startUrl: this.getNewFieldValue(
                "startUrl",
                fieldsToIgnore,
                oldTwaManifest.startUrl,
                fullStartUrl.pathname + fullStartUrl.search
            ),
            iconUrl: iconUrl || oldTwaManifestJson.iconUrl,
            maskableIconUrl:
                maskableIconUrl || oldTwaManifestJson.maskableIconUrl,
            monochromeIconUrl:
                monochromeIconUrl || oldTwaManifestJson.monochromeIconUrl,
            shortcuts: shortcuts,
        });
        return twaManifest;
    }
