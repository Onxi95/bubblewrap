...
export class ImageHelper {
    private async saveIcon(
        ...
        if (backgroundColor && image.hasAlpha()) {
            ...
            const replacementColor =
                ((backgroundColor.rgbNumber() << 8) & 0xffffff00) >>> 0;
    ...
    async fetchIcon(
        ...
        const response = await fetchUtils.fetch(iconUrl, login, password);
        ...
        if (!contentType?.startsWith("image/")) {
            throw new Error(
                `Received icon "${iconUrl}" with invalid Content-Type.` +
