...
const DELETE_FILE_LIST = ["app/src/main/res/mipmap-anydpi-v26/ic_launcher.xml"];
...
const NOTIFICATION_IMAGES: IconDefinition[] = [
    {
        dest: "app/src/main/res/drawable-mdpi/ic_notification_icon.png",
        size: 24,
    },
    {
        dest: "app/src/main/res/drawable-hdpi/ic_notification_icon.png",
        size: 36,
    },
    {
        dest: "app/src/main/res/drawable-xhdpi/ic_notification_icon.png",
        size: 48,
    },
    {
        dest: "app/src/main/res/drawable-xxhdpi/ic_notification_icon.png",
        size: 72,
    },
    {
        dest: "app/src/main/res/drawable-xxxhdpi/ic_notification_icon.png",
        size: 96,
    },
];
...
function shortcutMaskableTemplateFileMap(
    assetName: string
...
function shortcutMonochromeTemplateFileMap(
    assetName: string
): Record<string, string> {
    return {
...
function shortcutImages(assetName: string): IconDefinition[] {
    return [
        ...
        {
            dest: `app/src/main/res/drawable-xxhdpi/${assetName}.png`,
            size: 144,
        },
        {
            dest: `app/src/main/res/drawable-xxxhdpi/${assetName}.png`,
            size: 192,
        },
    ];
}
...
class Progress {
    ...
    update(): void {
        if (this.current === this.total) {
            throw new Error(
                "Progress already reached total." +
     ...
     */
        ...
        if (this.current !== this.total) {
            throw new Error(
                "Invoked done before current equals total." +
                    ` current: ${this.current}, total: ${this.total}`
            );
        }
    }
...
export class TwaGenerator {
    ...
    private async generateIcons(
        ...
        const icon = await this.imageHelper.fetchIcon(iconUrl, this.login, this.password);
        await Promise.all(
            iconList.map((iconDef) => {
                return this.imageHelper.generateIcon(
                    icon,
                    targetDir,
                    iconDef,
                    backgroundColor
    ...
    private async writeWebManifest(
        ...
        const response = await fetchUtils.fetch(
            twaManifest.webManifestUrl.toString(),
            this.login,
            this.password,
        );
        if (response.status !== 200) {
            throw new Error(
                `Failed to download Web Manifest ${twaManifest.webManifestUrl}.` +
        ...
        const webManifestLocation = path.join(
            targetDirectory,
            WEB_MANIFEST_LOCATION
        );

        // Ensures the target directory exists.
        await fs.promises.mkdir(webManifestLocation, { recursive: true });

        const webManifestFileName = path.join(
            webManifestLocation,
            WEB_MANIFEST_FILE_NAME
        );
        await fs.promises.writeFile(
            webManifestFileName,
            JSON.stringify(webManifestJson)
        );
    }
    ...
    private async generateShortcuts(
        ...
        await Promise.all(
            twaManifest.shortcuts.map(
                async (shortcut: ShortcutInfo, i: number) => {
                    ...
                    if (shortcut.chosenMonochromeIconUrl) {
                        await this.applyTemplateMap(
                            templateDirectory,
                            targetDirectory,
                            shortcutMonochromeTemplateFileMap(assetName),
                            templateArgs
                        );

                        const monochromeImages =
                            shortcutImages(monochromeAssetName);

                        const baseMonochromeIcon =
                            await this.imageHelper.fetchIcon(
                                shortcut.chosenMonochromeIconUrl,
                                this.login,
                                this.password,
                            );
                        const monochromeIcon =
                            await this.imageHelper.monochromeFilter(
                                baseMonochromeIcon,
                                twaManifest.themeColor
                            );

                        return await Promise.all(
                            monochromeImages.map((iconDef) => {
                                return this.imageHelper.generateIcon(
                                    monochromeIcon,
                                    targetDirectory,
                                    iconDef
                    ...
                    if (shortcut.chosenMaskableIconUrl) {
                        await this.applyTemplateMap(
                            templateDirectory,
                            targetDirectory,
                            shortcutMaskableTemplateFileMap(assetName),
                            templateArgs
                        );

                        const maskableImages =
                            shortcutImages(maskableAssetName);
                        await this.generateIcons(
                            shortcut.chosenMaskableIconUrl,
                            targetDirectory,
                            maskableImages
                        );
                    }
    ...
    private static generateShareTargetIntentFilter(
        ...
        if (twaManifest.shareTarget?.params?.files) {
            shareTargetIntentFilter.actions.push(
                "android.intent.action.SEND_MULTIPLE"
            );
            for (const file of twaManifest.shareTarget.params.files) {
                file.accept.forEach((accept) =>
                    shareTargetIntentFilter.mimeTypes.push(accept)
                );
            }
        }
     ...
     * @param {Object} twaManifest configurations values for the project.
     */
    async createTwaProject(
        targetDirectory: string,
        twaManifest: TwaManifest,
        log: Log,
        reportProgress: twaGeneratorProgress = noOpProgress
    ): Promise<void> {
        ...
        const args = {
            ...twaManifest,
            ...features,
            shareTargetIntentFilter:
                TwaGenerator.generateShareTargetIntentFilter(twaManifest),
        ...
        const iconOrMonochromeIconUrl =
            twaManifest.monochromeIconUrl || twaManifest.iconUrl;
        if (twaManifest.enableNotifications && iconOrMonochromeIconUrl) {
            await this.generateIcons(
                iconOrMonochromeIconUrl,
                targetDirectory,
                NOTIFICATION_IMAGES
            );
        }
    ...
    async removeTwaProject(targetDirectory: string): Promise<void> {
        await Promise.all(
            DELETE_PROJECT_FILE_LIST.map((entry) =>
                rmdir(path.join(targetDirectory, entry))
            )
        );
    }
}
