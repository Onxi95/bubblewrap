...
async function confirmTwaConfig(
    ...
    if (twaManifest.shortcuts.length > 0) {
        const addShortcuts = await prompt.promptConfirm(
            messages.promptShortcuts,
            true
        );
        if (!addShortcuts) {
    ...
    twaManifest.monochromeIconUrl = monochromeIconUrl
        ? monochromeIconUrl.toString()
        : undefined;

    const playBillingEnabled = await prompt.promptConfirm(
        messages.promptPlayBilling,
        false
    );
    ...
    const locationDelegationEnabled = await prompt.promptConfirm(
        messages.promptLocationDelegation,
        false
    );
...
async function createSigningKey(
    ...
    prompt.printMessage(
        messages.messageSigningKeyNotFound(twaManifest.signingKey.path)
    );
    ...
    const fullName = await prompt.promptInput(
        messages.promptKeyFullName,
        null,
        createValidateString(1)
    );
    const organizationalUnit = await prompt.promptInput(
        messages.promptKeyOrganizationalUnit,
        null,
        createValidateString(1)
    );
    const organization = await prompt.promptInput(
        messages.promptKeyOrganization,
        null,
        createValidateString(1)
    );
    const country = await prompt.promptInput(
        messages.promptKeyCountry,
        null,
        createValidateString(2, 2)
    );
    const keystorePassword = await prompt.promptPassword(
        messages.promptKeystorePassword,
        createValidateString(6)
    );
    const keyPassword = await prompt.promptPassword(
        messages.promptKeyPassword,
        createValidateString(6)
    );
...
export async function init(
    ...
    console.log("args manifest", args.manifest, args.login, args.password);
    ...
    if (!fs.existsSync(targetDirectory)) {
        ...
        if (
            !(await prompt.promptConfirm(
                messages.promptCreateDirectory(targetDirectory),
                true
            ))
        ) {
            throw new Error(
                messages.errorDirectoryDoesNotExist(targetDirectory)
            );
        }
        fs.promises.mkdir(targetDirectory, { recursive: true });
    }

    let twaManifest = await TwaManifest.fromWebManifest(
        args.manifest,
        args.login,
        args.password
    );
    ...
    twaManifest.signingKey.path = join(
        targetDirectory,
        twaManifest.signingKey.path
    );
    twaManifest = await confirmTwaConfig(twaManifest, prompt);
    const twaGenerator = new TwaGenerator(args.login, args.password);
    ...
    await generateManifestChecksumFile(
        join(targetDirectory, "/twa-manifest.json"),
        targetDirectory
    );
